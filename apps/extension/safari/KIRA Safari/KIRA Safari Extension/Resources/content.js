...
function rememberPointerCapture(event) {
...
function handlePointerMoveProbe(event) {
    if (pointerDragWindowOpened || !lastPointerCapture || !lastPointerPosition)
        return;
...
function handleDocumentDragProbe(event) {
    if (!draggingCapture && !hasLikelyCaptureDrag(event.dataTransfer))
        return;
    if (!draggingCapture) {
        showDropPad({ x: event.clientX, y: event.clientY });
        return;
...
function handleAltContextMenu(event) {
    if (!event.altKey)
        return;
...
function imageFromDragEvent(event) {
...
function captureFromDragEvent(event) {
...
function captureFromElementBackground(element) {
    for (const candidate of [element, ...Array.from(element.closest('a, [role="link"], [data-test-id], [data-testid]')?.querySelectorAll('*') ?? [])]) {
        ...
        if (!backgroundUrl || !isCaptureUrl(backgroundUrl))
            continue;
...
function firstCssImageUrl(backgroundImage) {
...
function captureFromImage(image) {
...
function bestImageUrlFromImage(image) {
...
function bestUrlFromSourceElements(sources) {
...
function parseSrcset(srcset) {
    return srcset
        ...
        const result = { url: parts[0] };
        for (const descriptor of parts.slice(1)) {
            const value = Number.parseFloat(descriptor.slice(0, -1));
            if (descriptor.endsWith('w') && Number.isFinite(value))
                result.width = value;
            if (descriptor.endsWith('x') && Number.isFinite(value))
...
function hasLikelyCaptureDrag(dataTransfer) {
    if (!dataTransfer)
        return false;
    if (draggingCapture)
        return true;
    const types = [...dataTransfer.types];
    return types.some((type) => ['application/x-kira-capture', 'text/html', 'text/uri-list', 'Files'].includes(type));
}
...
function updateNativeDragProgress(event) {
    if (!dragStartPosition) {
        dragStartPosition = { x: event.clientX, y: event.clientY };
    }
    const position = { x: event.clientX, y: event.clientY };
    if (lastDragPosition?.x === position.x && lastDragPosition.y === position.y)
        return;
...
function positionDropPad(pad, position) {
...
function wireCaptureWindow(pad) {
...
async function refreshCaptureContext(options = {}) {
    const now = Date.now();
    if (!options.force && now - lastContextRefreshAt < 900)
        return;
...
function renderCaptureContext() {
    ...
    if (!pad)
        return;
    const title = pad.querySelector('[data-role="file-title"]');
    const nodeList = pad.querySelector('[data-role="node-list"]');
    if (title)
        title.textContent = currentContext?.fileTitle || 'Open KIRA to target nodes';
    if (!nodeList)
        return;
    nodeList.innerHTML = '';
...
function renderComboOptions() {
    const pad = document.getElementById(dropPadId);
    const input = pad?.querySelector('.kira-capture-create input');
    const list = pad?.querySelector('.kira-capture-combo-list');
    if (!input || !list)
        return;
...
function extractCaptureFromDrop(dataTransfer) {
    if (!dataTransfer)
        return null;
    const kiraCapture = dataTransfer.getData('application/x-kira-capture');
    if (kiraCapture) {
        try {
            const parsed = JSON.parse(kiraCapture);
            if (parsed.kiraCapture === 1 && parsed.kind === 'image')
                return parsed;
        }
        catch {
...
async function enlargePinterestUrl(value) {
    let url;
    try {
    ...
    if (!url.hostname.includes('pinimg.com'))
        return value;
    if (!/\.(jpg|png|webp)(?:$|\?)/i.test(url.href))
        return value;
    if (url.href.includes('75x75_RS'))
        return value;
    const base = url.href
    ...
    const candidates = ['jpg', 'png', 'webp', 'gif'].map((candidate) => base.replace(new RegExp(`\\.${extension}(?=$|\\?)`, 'i'), `.${candidate}`));
