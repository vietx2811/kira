...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (!isOpenDragWindowMessage(message)) return
...
function isOpenDragWindowMessage(message: unknown): message is { type: 'kira-open-drag-window'; capture: PendingCapture } {
...
async function postCapture(capture: PendingCapture) {
  try {
