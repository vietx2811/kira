...
3. **Danh sách và chi tiết provider**, layout 2 cột giữ nguyên (`210px` / phần còn lại), nhưng:
   ...
   - **Hàng đang chọn, vi phạm thật tìm thấy khi rà lại wireframe [code]:** `.provider-list-row[aria-pressed="true"] { background: var(--glass-active) }` (`styles.css:5475-5477`) tô teal cho provider đang xem, đúng kiểu tint mà DESIGN.md §3 The Ink Primary Rule vừa thu hẹp phạm vi của teal (chỉ còn canvas, focus, trạng thái). Bản nháp đầu của wireframe này (`ai-providers.html`) cũng lỡ chép lại cùng màu teal đó cho khung "Đúng, đề xuất", đã sửa. **Đã chốt 2026-09-15:** hàng đang chọn dùng mực, không dùng nền tint: `box-shadow: inset 3px 0 0 0 var(--text-main)` ở mép trái, chữ tên provider weight 680, không đổi nền. Cùng họ với viền mực 1,5px của nút secondary selected (§7 bảng 4 cấp).
